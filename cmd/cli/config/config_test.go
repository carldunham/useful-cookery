...
func TestLoad(t *testing.T) {
	...
	t.Setenv("UC_AUTH_TOKEN_EXPIRY", "48h")
	...
	if cfg.Auth.JWTSecret != "test-secret" {
		t.Errorf("Expected Auth.JWTSecret = test-secret, got %s", cfg.Auth.JWTSecret)
	}
	if cfg.Auth.TokenExpiry != 48*time.Hour {
		t.Errorf("Expected Auth.TokenExpiry = 48h, got %s", cfg.Auth.TokenExpiry)
	}
}
